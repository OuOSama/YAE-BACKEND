...
**Next-gen VTuber AI character platform**  
*Built with Elysia.js, Bun & PostgreSQL*
...
| Feature | Description |
|---------|-------------|
| 🤖 **AI Chat** | Yae Miko roleplay with vLLM function calling |
| 🔐 **Dual Auth** | OAuth (Discord) + M2M service tokens with scopes |
| ⚡ **Real-time** | WebSocket pub/sub for service status |
| 🚀 **Fast** | Bun runtime with hot-reload development |
...
```
📦 src/
 ├─ 🤖 modules/ai/        → Chat & RAG endpoints
 ├─ 🔐 modules/auth/      → OAuth & M2M tokens
 ├─ 📡 modules/broadcast/ → WebSocket pub/sub
 ├─ 🗄️  database/         → Drizzle ORM schemas
 └─ 🛡️  lib/auth/         → Auth utilities
```
...
| Module | Endpoints |
|--------|-----------|
| **AI** | `POST /api/ai/chat` · `POST /api/ai/rag` |
| **Auth** | `POST /auth/*` · `POST /get-access` |
| **Broadcast** | `ws://localhost:3001/broadcast` |
...
```
User Auth        → Discord OAuth + Email/Password (better-auth)
Service Auth     → x-service-key header → JWT token (7d expiry)
Authorization    → Bearer token + scopes (ai:write, etc.)
```
...
| **Service Keys** | Currently hardcoded → TODO: DB-stored with rotation |
